...
def upload_image():
    ...
    elif operation_selection == 'water_color':
...
def watercolor_effect(decode_array_to_img):
    ...
    wce=cv2.stylization(decode_array_to_img, sigma_s=60, sigma_r=0.6)
    status, output_img = cv2.imencode('.PNG', wce)

    return output_img

def invert(decode_array_to_img):

    ivr=cv2.bitwise_not(decode_array_to_img)
    status, output_img = cv2.imencode('.PNG', ivr)

    return output_img

def HDR(decode_array_to_img):

    hdr=cv2.detailEnhance(decode_array_to_img, sigma_s=12, sigma_r=0.15)
